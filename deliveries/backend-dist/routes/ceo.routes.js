...
const checkExecutiveAccess = (req, res, next) => {
...
router.get('/company-overview', async (req, res) => {
    try {
        const okrs = await prisma_service_1.default.strategicOKR.findMany();
        res.json({
            healthScore: 92,
            subsidiaries: [
...
router.get('/finance', async (req, res) => {
    try {
        const revenues = await prisma_service_1.default.revenueStream.findMany({ orderBy: { createdAt: 'asc' } });
        res.json({
...
router.get('/operations', async (req, res) => {
    try {
        const risks = await prisma_service_1.default.riskAssess.findMany();
        res.json({
            operationalKPIs: {
                uptime: 99.98,
...
router.get('/projects', async (req, res) => {
    try {
        const projects = await prisma_service_1.default.project.findMany();
...
router.get('/product', async (req, res) => {
    try {
        const products = await prisma_service_1.default.productUsage.findMany();
        res.json({
...
router.get('/investor', async (req, res) => {
    try {
        const reports = await prisma_service_1.default.investorReport.findFirst();
        res.json(reports || {
...
router.post('/ai-assistant', async (req, res) => {
    try {
        const parsed = zod_1.z.object({ query: zod_1.z.string() }).safeParse(req.body);
        if (!parsed.success)
            return res.status(400).json({ message: 'Missing query parameter.' });
        const query = parsed.data.query.toLowerCase();
        const widgets = await prisma_service_1.default.aIWidgetData.findFirst();
...
router.get('/notifications', async (req, res) => {
    try {
        const list = await prisma_service_1.default.systemNotification.findMany({ orderBy: { createdAt: 'desc' } });
        res.json(list);
    }
    catch (err) {
        res.status(500).json({ message: err.message });
...
router.post('/org-structure', async (req, res) => {
    try {
        const parsed = zod_1.z.object({
            type: zod_1.z.enum(['subsidiary', 'bu']),
            name: zod_1.z.string(),
            locationOrLead: zod_1.z.string(),
            headcountOrTeams: zod_1.z.string(),
            focusOrStatus: zod_1.z.string()
        }).parse(req.body);
...
router.post('/settings', async (req, res) => {
    try {
        const parsed = zod_1.z.object({
            telemetryExempt: zod_1.z.boolean(),
            screenSamplingRateSec: zod_1.z.number(),
            securityLoggingLevel: zod_1.z.string(),
            anomalyDetectionSensitivity: zod_1.z.number()
        }).parse(req.body);
        settingsStore = {
            ...settingsStore,
            ...parsed
...
router.post('/strategic', async (req, res) => {
    try {
        const parsed = zod_1.z.object({
            ...
            keyResult: zod_1.z.string(),
            targetValue: zod_1.z.number(),
            currentValue: zod_1.z.number(),
            owner: zod_1.z.string(),
            quarter: zod_1.z.string(),
            year: zod_1.z.number(),
            risks: zod_1.z.string()
        }).parse(req.body);
        const okr = await prisma_service_1.default.strategicOKR.create({
            data: {
                ...parsed,
...
router.post('/risk', async (req, res) => {
    try {
        const parsed = zod_1.z.object({
            riskCategory: zod_1.z.string(),
            riskDescription: zod_1.z.string(),
            severity: zod_1.z.string(),
            mitigationPlan: zod_1.z.string(),
            status: zod_1.z.string(),
            department: zod_1.z.string()
        }).parse(req.body);
        const risk = await prisma_service_1.default.riskAssess.create({
            data: parsed
        });
        res.json(risk);
    }
